    ...
    while (dead == 0) {
      ...
      ask:
        ...
        if (in == 'E') {
          ...
          if (((u.i[1] == 1 || u.c[UC_STRENGTH] == 3) && (u.i[2] == 1 || u.i[2] == 3))
            ...
            if (tmp < 1)
              tmp = 1;
            if (roll(2,10) <= u.c[UC_DEX] && sqrt(tmp/25.0)*25.0 >=
                roll(1,25)) {
              printf("You made it!\r\n");
              ok = 0;
              while (ok == 0) {
        ...
        if (in == 'F') {
          printf("Fight\r\n");
          i1 = 3 * u.c[UC_DEX] + u.c[UC_STRENGTH] + u.c[UC_WEAPON] + u.c[UC_LEVEL] * (5 - u.c[UC_CLASS])
            - m1 * mm[m].m / 2.0;
