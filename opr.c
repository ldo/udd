...
otop:
        ...
        case 'R':
            ...
            if (!fgets(nbuf, NAMELEN, stdin) || sscanf(nbuf, "%d%d", &dgn, &lvl) != 2)
                dgn = -1;
            unix_tty_dgn();
            fflush(stdin);
            if (dgn < 1 || dgn > n_dlvl)
            ...
            while
              (
                    (tmp = lock_open(FIL_LVL, O_RDWR, FIL_LVL_LD, FIL_LVL_LK, 60)) < 0
                &&
                    (tmp == -2)
              )
              {
            ...
            lseek
              (
                ...
                /*offset =*/
                        sizeof(n_dlvl)
                    +
                        (dgn - 1) * (DNAM_SZ + sizeof(dd.dstart) +  8000)
                    +
                        DNAM_SZ
                    +
                        sizeof(dd.dstart)
                    ...
                    if (tmp2 == 'X')
                        ...
                        fprintf
                          (
                            fp,
                            " %s",
                            (((dd.dmap[(tmp - 1) * 20 + (lcv - 1)] / 4) & 3) == 1) ? "_" : " "
                          );
                fprintf(fp, "\r\n");
                for (lcv = 1 ; lcv <= 20 ; lcv++)
                  {
                    fprintf(fp,"%1.1s", " |!:" +  (dd.dmap[(tmp - 1) * 20 + (lcv - 1)] & 3));
                    if (dd.dmap[(tmp - 1) * 20 + (lcv - 1)] / 16 == 0)
                        ...
                        fprintf
                          (
                            fp,
                            "%1.1s",
                            " abcdefghijklmnop" +  dd.dmap[(tmp - 1) * 20 + (lcv - 1)] / 16
                          );
                  } /*for*/
        ...
        case 'D':
            ...
            while
              (
                    (tmp3 = lock_open(FIL_LVL, O_RDWR, FIL_LVL_LD, FIL_LVL_LK, 60)) < 0
                &&
                    (tmp3 == -2)
              )
              {
        ...
        case 'C':
            ...
            while
              (
                    (tmp3 = lock_open(buf, O_RDWR, FIL_CHR_LD, FIL_CHR_LK, 60)) < 0
                &&
                    (tmp3 == -2)
              )
              {
        ...
        case 'N':
            ...
            tmp = atoi(buf);
            if (tmp < 0 || (tmp2 == 'A' && tmp > 64) || (tmp2 != 'A' && tmp > 1))
              {
                printf("Out of range!\r\n");
                break;
              } /*if*/
